...
# PowerShell Core script for restoring the latest backup of UserDB
Write-Host "Starting the script to restore the latest backup for UserDB..."
...
# Find the latest backup file for UserDB
Write-Host "Searching for the latest UserDB backup..."
$backupFiles = Get-ChildItem -Path $backupDir -Filter "UserDB_*.bak" | Sort-Object LastWriteTime -Descending | Select-Object -First 1
if (-not $backupFiles) {
    Write-Error "ERROR: No backup files found for UserDB in ${backupDir}"
    Read-Host "Press Enter to continue..."
    exit 1
}
