...
# PowerShell Core script for restoring the latest backup of RightsDB
Write-Host "Starting the script to restore the latest backup for RightsDB..."
...
# Find the latest backup file for RightsDB
Write-Host "Searching for the latest RightsDB backup..."
$backupFiles = Get-ChildItem -Path $backupDir -Filter "RightsDB_*.bak" | Sort-Object LastWriteTime -Descending | Select-Object -First 1
if (-not $backupFiles) {
    Write-Error "ERROR: No backup files found for RightsDB in ${backupDir}"
    Read-Host "Press Enter to continue..."
    exit 1
}
