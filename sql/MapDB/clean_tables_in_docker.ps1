...
<#
.SYNOPSIS
    Очищает таблицы в указанной базе данных
#>
...
$SQL_SCRIPT_NAME = "clean_tables.sql"
...
$confirmation = Read-Host "`nWARNING: This will DELETE ALL DATA from database '$DATABASE'. Are you sure? (y/n)"
...
if ($scriptResult -eq $false) {
    Write-Output "ERROR: Database cleanup failed"
    exit 1
}
...
Write-Output "`nDatabase cleanup completed successfully"
exit 0