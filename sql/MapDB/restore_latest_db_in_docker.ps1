...
# PowerShell Core script for restoring the latest backup of MapDB
Write-Host "Starting the script to restore the latest backup for MapDB..."
...
# Find the latest backup file for MapDB
Write-Host "Searching for the latest MapDB backup..."
$backupFiles = Get-ChildItem -Path $backupDir -Filter "MapDB_*.bak" | Sort-Object LastWriteTime -Descending | Select-Object -First 1
if (-not $backupFiles) {
    Write-Error "ERROR: No backup files found for MapDB in ${backupDir}"
    Read-Host "Press Enter to continue..."
    exit 1
}
