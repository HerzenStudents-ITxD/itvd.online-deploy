...
# PowerShell Core script for restoring the latest backup of FeedbackDB
Write-Host "Starting the script to restore the latest backup for FeedbackDB..."
...
# Find the latest backup file for FeedbackDB
Write-Host "Searching for the latest FeedbackDB backup..."
$backupFiles = Get-ChildItem -Path $backupDir -Filter "FeedbackDB_*.bak" | Sort-Object LastWriteTime -Descending | Select-Object -First 1
if (-not $backupFiles) {
    Write-Error "ERROR: No backup files found for FeedbackDB in ${backupDir}"
    Read-Host "Press Enter to continue..."
    exit 1
}
