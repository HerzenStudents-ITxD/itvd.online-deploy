...
# PowerShell Core script for configuring the UserDB database
Write-Host "Starting the UserDB database population script..."
...
$requiredVars = @("DB_CONTAINER", "SA_PASSWORD", "USERDB_DB_NAME")
$missingVars = @()
...
if ($database -ne "UserDB") {
    Write-Error "ERROR: Database name ($database) does not match expected 'UserDB'."
    exit 1
}
...
$sqlScriptPath = Join-Path -Path $scriptDir -ChildPath "UserDB/09_add_users.sql"
...
try {
    docker cp $sqlScriptPath "${container}:/tmp/09_add_users.sql"
    if ($LASTEXITCODE -ne 0) {
...
# Execute SQL script
Write-Host "Configuring UserDB tables and data..."
$setupQuery = "/opt/mssql-tools/bin/sqlcmd -S localhost -U SA -P '$($password -replace "'", "''")' -d $database -i /tmp/08_setup_UserDB.sql"
...
# Verify UserDB tables
Write-Host "Verifying UserDB tables..."
$verifyScriptPath = Join-Path -Path $scriptDir -ChildPath "UserDB/check_tables_in_docker.ps1"
...
Write-Host "UserDB setup completed successfully ✅"
exit 0